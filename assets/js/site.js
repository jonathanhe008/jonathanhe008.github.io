...
function initPostEngagement() {
  ...
  if (btn && countEl) {
    ...
    render();
    btn.addEventListener('click', () => {
      // TODO(jhez): replace with API call — POST /api/upvote { slug, action }
      if (voted) { voted = false; count = Math.max(0, count - 1); }
      else       { voted = true;  count = count + 1; }
      try {
        localStorage.setItem(key + ':voted', voted ? '1' : '0');
        localStorage.setItem(key + ':count', String(count));
      } catch (_) {}
      render();
      console.log('[upvote]', { slug, voted, count });
    });
  }
  ...
  if (form && list) {
    // TODO(jhez): on load, fetch server-side comments and render into list.
    form.addEventListener('submit', (ev) => {
      ...
      if (!name || !body) return;
      // TODO(jhez): POST { slug, name, body } to backend; optimistic UI below.
      const item = document.createElement('li');
      item.className = 'comment';
      const meta = document.createElement('div');
      meta.className = 'comment__meta';
      meta.textContent = name + ' · just now';
      ...
      form.reset();
      console.log('[comment]', { slug, name, body });
    });
  }
}
